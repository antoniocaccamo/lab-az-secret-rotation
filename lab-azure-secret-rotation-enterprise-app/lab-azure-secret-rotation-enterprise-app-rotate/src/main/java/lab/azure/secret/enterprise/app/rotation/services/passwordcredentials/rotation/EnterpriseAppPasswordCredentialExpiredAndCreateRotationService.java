 ...
 *                              |   |
 *   *******+**********+++*     |   |
 *   |      expired       |     |   |     -> | 1. delete old {@link PasswordCredential}
 *   *******+**********+++*     |   |        | 2. create a new {@link PasswordCredential}
 *                              |   |        | 3. store on blob
 *                              |   |        | 4. assign role on storage account and container
 *                              |   |        | 5. inform
 *                              |   |
 *                              |   |
 *                              |   |
...
@ApplicationScoped
public class EnterpriseAppPasswordCredentialExpiredAndCreateRotationService extends AbstractPasswordCredentialRotationService {

    protected static final String TemplatePath = "email/EnterpriseApp.PasswordCredentialExpiredAndCreate.ftl";

    @Inject
    @TemplatePath(TemplatePath)
    Template template ;

    public EnterpriseAppPasswordCredentialExpiredAndCreateRotationService() {
        super(EnterpriseAppEventEnum.PasswordCredentialExpiredAndCreate);
    }
